...
export class NewGroupPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public imageProvider: ImageProvider, public dataProvider: DataProvider, public formBuilder: FormBuilder,
    public alertProvider: AlertProvider, public alertCtrl: AlertController, public angularfire: AngularFire, public app: App, public loadingProvider: LoadingProvider) {
  ...
  // Proceed with group creation.
    ...
    this.angularfire.database.list('groups').push(this.group).then((success) => {
      let groupId = success.key;
      // Add group reference to users.
      this.angularfire.database.object('/accounts/' + this.groupMembers[0].$key + '/groups/' + groupId).update({
        messagesRead: 1
      });
      for (var i = 1; i < this.groupMembers.length; i++) {
        this.angularfire.database.object('/accounts/' + this.groupMembers[i].$key + '/groups/' + groupId).update({
          messagesRead: 0
        });
      }
    ...
    this.alert = this.alertCtrl.create({
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: () => {
            this.imageProvider.setGroupPhoto(this.group, Camera.PictureSourceType.PHOTOLIBRARY);
          }
        },
        {
          text: 'Take Photo',
          handler: () => {
            this.imageProvider.setGroupPhoto(this.group, Camera.PictureSourceType.CAMERA);
          }
        }
      ]
    }).present();
